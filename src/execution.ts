 ...
 */
export function portGraph(graph: unknown): PortGraph {
	const asGraph = (graph ?? {}) as { agents?: unknown; connections?: unknown };
	const agents = Array.isArray(asGraph.agents) ? asGraph.agents : [];
	const connections = Array.isArray(asGraph.connections) ? asGraph.connections : [];

	const ids: string[] = [];
	const byId: Record<string, PortNode> = {};
	...
	for (const agent of agents) {
		...
		const node: PortNode = { id, inputs: [], outputs: [], inputById: {}, outputById: {} };
		...
		if (Array.isArray(rec.inputPorts)) {
			...
			const port: ResolvedInputPort = { name: "in", portId, policy: "all-of", edges: [], sources: [] };
			node.inputs.push(port);
		...
		if (Array.isArray(rec.outputPorts)) {
			...
			for (const name of rec.outputPorts) {
				...
				seen.add(name);
				const port: ResolvedOutputPort = { name, portId: `${id}:${name}`, edges: [], targets: [] };
			...
			const port: ResolvedOutputPort = { name: "out", portId, edges: [], targets: [] };
			node.outputs.push(port);
	...
	for (const conn of connections) {
		const rec = conn as { id?: unknown; source?: unknown; target?: unknown; sourcePort?: unknown; targetPort?: unknown } | null | undefined;
		if (rec == null || typeof conn !== "object") continue;
 ...
 */
export function evaluateBindings(bindings: readonly OutputBinding[] | undefined | null, structured: unknown, count?: number): string | null {
	if (!Array.isArray(bindings) || bindings.length === 0) return null;
	const hasStructured = structured !== undefined && structured !== null;
	const record = hasStructured && typeof structured === "object" ? (structured as Record<string, unknown>) : {};
	for (const binding of bindings) {
		...
		if (field === COUNT_KEY) {
			...
			if (matchesOp(binding, count)) return port;
			continue;
		}
		...
		if (matchesOp(binding, record[field])) return port;
	}
	return null;
}

/** One row's comparison: "==" (the default, with the String-coerced fallback) or the numeric ">=". */
function matchesOp(binding: OutputBinding, actual: unknown): boolean {
	if (binding.op === ">=") {
