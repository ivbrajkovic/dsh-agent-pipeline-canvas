...
export function validateGraph(graph: unknown): ValidationResult {
	...
	const agentById = new Map<string, unknown>();
	...
	for (const agent of agents) {
		...
		const rec = agent as { id?: unknown; input?: unknown; output?: unknown };
	...
	function inputPort(agent: unknown): string {
		const rec = agent as { id?: unknown; input?: unknown } | null | undefined;
		if (rec != null && typeof rec.input === "string" && rec.input.length > 0) return rec.input;
		return `${rec != null && rec.id != null ? String(rec.id) : ""}:in`;
	}
	function outputPort(agent: unknown): string {
		const rec = agent as { id?: unknown; output?: unknown } | null | undefined;
