...
import { renderValue } from "./execution.ts";
 ...
 */
export function composePipelineInput(text: string, files: readonly string[]): string {
	const parts: string[] = [];
 ...
 */
export function finalOutputText(outputs: Record<string, unknown>, labelOf: (id: string) => string): string {
	const map = outputs ?? {};
