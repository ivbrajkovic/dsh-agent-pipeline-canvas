...
import { sessionPipelineFilePath, writeAtomic } from "./storage.ts";
 ...
 */
async function readPipelineFile(path: string): Promise<{ found: boolean; pipeline: unknown }> {
	try {
		...
		handler: async (req, res) => {
			...
			try {
				if (method === "GET") {
					...
					if (read === null || !read.found) read = await readPipelineFile(path);
