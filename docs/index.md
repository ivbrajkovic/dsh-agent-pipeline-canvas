...
| [proposals/node-context-menu.md](proposals/node-context-menu.md) | **Planned.** The node context menu — right-click on an agent node for **Go to transcript** (the projected child session) plus the node actions, on the harness `Menu` primitive. Phased plan; living phase-delta handoff alongside. |
