...
| [guide/pipeline-samples.md](guide/pipeline-samples.md) | Short sample graphs: sequential chains, concurrent fan-out/fan-in, any-of joins, conditional routers via the if control (or hand-authored output bindings), the Reviewer loop in the if-authored form (a `$count` guard) with the ports+bindings bound-guarded twin, and steering at breakpoints. |
...
| [reference/graph-and-execution.md](reference/graph-and-execution.md) | The pipeline graph schema (ports, policies, bounds, bindings, the `$count`/`op` condition language, controls), every `validateGraph` rule and error code (the guard rule: every cycle carries its guard), the if control's lowering contract, the kernel's firing rules and emission contract, and the firing-log run record. |
...
| [proposals/loops.md](proposals/loops.md) | **Built.** Loops as the flowchart draws them — the if control with a branch wired backward, `$count` (the firing's own sequence) and `>=` in the condition language, a required guard on every cycle (`cycle-unguarded`), and the automatic `any-of` entry assist. Phased plan (L1–L4); living phase-delta handoff alongside. |
