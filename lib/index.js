...
//
//   GET  /dsh-agent-pipeline?cwd=<absolute project root>[&sessionId=<key>]
//        -> { ok, pipeline: <graph> | null, validation, run, lastRun }
//        (no file yet => null. With a valid `sessionId`: the session's own
//        graph when it has one, else read-through to the legacy workspace
//        file — a GET never forks; an invalid key answers 400. `run` carries
//        the active run record — running or paused — so a reload discovers it
//        without a list route; with a valid `sessionId` run discovery is
//        scoped to that session, without one it stays workspace-level.
//        `lastRun` is the newest record of
//        any state, null while a run is active — a remounted canvas restores
//        the last run's result from it.)
//   POST /dsh-agent-pipeline   body { cwd, graph, sessionId? }
//        -> { ok, validation }                     (writes the file; a
//        non-empty `sessionId` writes the session's own file — the fork —
//        leaving the legacy workspace file untouched)
...
//
// Run execution: the run registry (lib/runs.ts) drives the firing kernel
// concurrently. Non-breakpointed agents run as one-shot `spawn` subagents
// parented to the session agent; breakpointed agents run as continuable
// subagents under a hidden per-node parent anchor (`origin: "subagent"`,
// delegationDepth 0), pausing at each breakpoint for the user's Resume /
// Rerun / Steer / Abort. Breakpointed agents require the
...
// rerun still works.
//
// Storage: `<cwd>/.agent-pipeline/pipeline.json` (the legacy shared graph —
// fork source and the fallback every session without a file of its own reads
// through to), `<cwd>/.agent-pipeline/pipelines/<sessionId>.json` (a
// session's own graph, written by its first session-scoped save; a merely
// open session never forks), and `<cwd>/.agent-pipeline/runs/<runId>.json`.
// All written atomically (temp file + rename — see lib/storage.ts) so a crash
// mid-write never leaves a truncated file. The pipeline and its runs belong
// to the workspace / repo where the Agent Pipeline view is opened; a
// different repository has its own project root and therefore its own files.
//
...
import { sessionPipelineFilePath, writeAtomic } from "./storage.js";
const name = "agent-pipeline-canvas";
// `webServer` serves the routes; `agents` resolves the live session Agent that
// parents one-shot pipeline subagents (and seeds each node's parent anchor);
 ...
 * The per-session counterpart (`pipelines/<sessionId>.json`) lives in
 * lib/storage.ts.
...
/**
 * Buffer and decode one pipeline file: the parsed graph, or null when the
 * file is missing (ENOENT) or malformed. Both degrade to "no pipeline"
 * rather than failing the view; the next save rewrites the file cleanly.
 */
async function readPipelineFile(path) {
    try {
        const text = await readFile(path, "utf8");
        try {
            return { found: true, pipeline: JSON.parse(text) };
        }
        catch {
            return { found: true, pipeline: null };
        }
    }
    catch (error) {
        if (error.code !== "ENOENT")
            throw error;
        return { found: false, pipeline: null };
    }
}
...
export function apply(ctx) {
    ...
    const registry = new RunRegistry({
        ...
        logger: ctx.logger,
        subscribeRunEnd: (fn) => ctx.on("subagent/end", (info) => fn(info)),
        ...
        handler: async (req, res) => {
            ...
            try {
                if (method === "GET") {
                    ...
                    // Session scoping: a valid non-empty `sessionId` reads that
                    // session's own graph, falling back to the legacy workspace
                    // file while the session has none (read-through — a GET never
                    // forks, so merely opening the tab writes nothing). Absent or
                    // empty keeps the legacy behavior byte-identical; a present
                    // but invalid key is a client bug and answers 400.
                    const sessionId = url.searchParams.get("sessionId") ?? "";
                    let sessionPath = null;
                    if (sessionId.length > 0) {
                        sessionPath = sessionPipelineFilePath(cwd, sessionId);
                        if (sessionPath === null) {
                            send(res, 400, { ok: false, error: "invalid or missing sessionId" });
                            return;
                        }
                    }
                    // The session read only falls through to the legacy file on
                    // ENOENT; a malformed session file degrades to "no pipeline"
                    // exactly as the legacy file does.
                    let read = sessionPath !== null ? await readPipelineFile(sessionPath) : null;
                    if (read === null || !read.found)
                        read = await readPipelineFile(path);
                    ...
                    // `run` is the active run record (running|paused) — the discovery
                    // path for a page reload; loading it also sweeps stale runs and
                    // resurrects paused ones (see lib/runs.ts). A valid `sessionId`
                    // scopes that discovery to the session (its own active run and
                    // last run); absent/empty keeps the workspace-level legacy path.
                    ...
                    const runScope = sessionPath !== null ? sessionId : null;
                    const run = await registry.activeRunForCwd(cwd, runScope);
                    const lastRun = run === null ? await registry.latestRunForCwd(cwd, runScope) : null;
                ...
                if (method === "POST") {
                    ...
                    const legacyPath = pipelinePath(payload.cwd);
                    if (legacyPath === null) {
                        send(res, 400, { ok: false, error: "invalid or missing cwd" });
                        return;
                    }
                    // The fork: a save carrying a non-empty `sessionId` writes
                    // THAT session's own file and leaves the legacy workspace
                    // graph untouched; absent/empty keeps the legacy write as
                    // today; an invalid key answers 400.
                    const sessionId = payload.sessionId;
                    let path = legacyPath;
                    if (sessionId !== undefined && sessionId !== null && sessionId !== "") {
                        const sessionPath = sessionPipelineFilePath(payload.cwd, sessionId);
                        if (sessionPath === null) {
                            send(res, 400, { ok: false, error: "invalid or missing sessionId" });
                            return;
                        }
                    ...
                    // The directory of whichever file is about to be written
                    // (`.agent-pipeline` for the legacy graph,
                    // `.agent-pipeline/pipelines` for a session's own).
    ...
    ctx.effect(() => ctx.webServer.register({
        ...
        handler: async (req, res) => {
            ...
            try {
                ...
                const outcome = await registry.control(payload.runId, { action: payload.action, feedback: payload.feedback }, payload.cwd);
        ...
        handler: async (req, res) => {
            ...
            try {
                ...
                for (const entry of ctx.llm.listProviders()) {
                    if (entry == null || typeof entry.id !== "string" || entry.id.length === 0)
                        continue;
                    providers.push({
                        id: entry.id,
