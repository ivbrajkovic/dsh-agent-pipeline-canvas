...
// future runner will rely on. Given the persisted, already-validated graph
// (the same `{ agents, connections }` DAG defined by ./graph.js), it defines:
//
...
// imported by the Host and the future runner and exercised in a plain Node
// script (test/execution.test.mjs). Scheduling, retries, conditions, loops,
// model selection, tool configuration, and credentials are the runner's job and
 ...
 * @returns
 *   {
 *     agents:      string[]            // known agent ids, in graph order
 *     roots:       string[]            // in-degree 0 (includes orphans)
 *     terminals:   string[]            // out-degree 0 (includes orphans)
 *     orphans:     string[]            // in-degree 0 AND out-degree 0
 *     upstream:    { [id]: string[] }  // sorted, deduped upstream ids per agent
 *     downstream:  { [id]: string[] }  // sorted, deduped downstream ids per agent
 *   }
 */
export function classifyGraph(graph) {
	...
	for (const conn of connections) {
		...
		const source = idOf(conn.source);
		const target = idOf(conn.target);
	...
	for (const id of agentIds) {
		const ups = [...upstreamSet.get(id)].sort(cmp);
		const downs = [...downstreamSet.get(id)].sort(cmp);
		upstream[id] = ups;
		downstream[id] = downs;
...
function lookupAgent(agentById, id) {
	...
	if (typeof agentById.get === "function") return agentById.get(id);
	return agentById[id];
}
...
function sourceLabel(key, agentById) {
	...
	if (agent && typeof agent.name === "string" && agent.name.length > 0) return agent.name;
	return key;
}
