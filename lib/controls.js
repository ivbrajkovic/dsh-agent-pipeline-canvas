...
import { COUNT_KEY, isValuedRow } from "./execution.js";
 ...
 */
export function branchRows(branch) {
    if (branch == null || typeof branch !== "object")
        return [];
    const rec = branch;
    if (Array.isArray(rec.conditions)) {
        return rec.conditions.filter((c) => c != null && typeof c === "object" && !Array.isArray(c));
    }
    if (rec.field === undefined && rec.value === undefined && rec.op === undefined)
        return [];
    return [
        {
 ...
 */
export function validateControls(graph, agentIds, errors, warnings) {
    const ids = new Set();
    ...
    for (const conn of connections) {
        ...
        if (target.length > 0 && ids.has(target))
            incoming.set(target, (incoming.get(target) ?? []).concat([source]));
        if (source.length > 0)
    ...
    for (const control of controls) {
        const id = control.id;
        if (control.kind !== "if")
            continue; // a future kind owns its rules
        ...
        if (!agentIds.has(source))
            continue; // unknown source: validateGraph's connection pass (which runs after this one) reports connection-source-missing
...
function validateBranches(controlId, branches, errors) {
    ...
    branches.forEach((entry, index) => {
        ...
        if (branch.side !== undefined && !PORT_SIDES.includes(branch.side)) {
        ...
        rows.forEach((row, rowAt) => {
            ...
            if (row.op !== undefined && !BRANCH_OPS.includes(row.op)) {
...
function warnUnreachable(controlId, sourceId, owner, branches, warnings) {
    ...
    const schema = settings != null && typeof settings === "object" ? settings.outputSchema : undefined;
 ...
 */
export function lowerControls(graph) {
    ...
    for (const agent of agentsRaw) {
        const id = agent != null && typeof agent === "object" ? argStr(agent.id) : "";
        if (id.length > 0 && !agentIds.has(id))
            agentIds.add(id);
    }
    ...
    for (const entry of raw) {
        const id = entry != null && typeof entry === "object" ? argStr(entry.id) : "";
        if (id.length > 0)
            controlIds.add(id);
    }
    ...
    const firstAgentSource = (controlId) => {
        for (const conn of connectionsRaw) {
            if (conn == null || typeof conn !== "object")
                continue;
            const rec = conn;
            if (argStr(rec.target) !== controlId)
                continue;
            const source = argStr(rec.source);
    ...
    const lowerings = new Map();
    for (const entry of raw) {
        if (entry == null || typeof entry !== "object" || Array.isArray(entry))
            continue;
        const control = entry;
        const id = argStr(control.id);
        if (id.length === 0 || control.kind !== "if")
            continue;
        ...
        for (const branch of Array.isArray(control.branches) ? control.branches : []) {
            ...
            if (spec.side !== undefined && PORT_SIDES.includes(spec.side) && spec.side !== DEFAULT_BRANCH_SIDE) {
    ...
    for (const lowering of lowerings.values()) {
        if (!owners.has(lowering.owner))
            owners.set(lowering.owner, lowering);
    }
    const agents = agentsRaw.map((agent) => {
        const id = agent != null && typeof agent === "object" ? argStr(agent.id) : "";
        const lowering = owners.get(id);
        if (lowering === undefined || agent == null || typeof agent !== "object")
            return agent;
        const clone = { ...agent, outputPorts: [...lowering.outputPorts], bindings: lowering.bindings.map((b) => ({ ...b })) };
        if (Object.keys(lowering.sides).length > 0)
            clone.outputPortSides = { ...lowering.sides };
        else
            delete clone.outputPortSides;
        return clone;
    });
    const connections = [];
    for (const conn of connectionsRaw) {
        ...
        if (controlIds.has(target))
            continue; // the control's single input — gone with the control
        ...
        if (controlIds.has(source))
            continue; // a skipped control's edges vanish with it
        connections.push(conn);
    }
    return { agents: agents, connections };
}
/**
 ...
 */
export function firedBranches(branches, emittedTo) {
    if (!Array.isArray(emittedTo))
        return [];
    ...
    const declared = Array.isArray(branches)
        ? branches
            .map((branch) => (branch != null && typeof branch === "object" ? argStr(branch.name) : ""))
 ...
 */
export function countThreshold(branches) {
    if (!Array.isArray(branches))
        return null;
    for (const entry of branches) {
        if (entry == null || typeof entry !== "object")
            continue;
        for (const row of branchRows(entry)) {
            if (row.field !== COUNT_KEY || row.op !== ">=" || !isValuedRow(row))
                continue;
