// dsh-agent-pipeline-canvas — graph semantics (shared by the Host and the
// future runner; the browser half mirrors validateGraph for live UI feedback).
//
...
// so it can be imported by the Host (./index.js) and the future executor, and its
// behaviour can be exercised in a plain Node script (see test/validate.test.mjs).
//
...
export function validateGraph(graph) {
	...
	for (const conn of connections) {
		...
		const source = argStr(conn.source);
		const target = argStr(conn.target);
		const sourcePort = argStr(conn.sourcePort);
		const targetPort = argStr(conn.targetPort);
...
function findCycle(agentIds, connections) {
	...
	for (const conn of connections) {
		...
		const source = conn.source == null ? "" : String(conn.source);
		const target = conn.target == null ? "" : String(conn.target);
