...
import { INPUT_KEY, cmp, evaluateBindings } from "./execution.js";
...
export class Kernel {
    ...
    constructor(graph, options = {}) {
        ...
        for (const id of graph.ids) {
            ...
            this.ports.set(id, node.inputs.map((spec) => ({
                spec,
                queue: [],
                sources: sourceFed
                    ? [SOURCE_NODE_ID]
     ...
     */
    deliver(targetId, targetPortId, message) {
     ...
     */
    selectEmissionPorts(nodeId, structured, seq) {
        const ports = this.outPorts.get(nodeId);
        if (ports === undefined)
            return [];
    ...
    starvingCandidates() {
        const waiting = [];
        for (const [id, ports] of this.ports) {
            if (this.firedNodes.has(id))
                continue;
            if (ports.length === 0 || ports.some((port) => port.sources.length > 0))
                waiting.push(id);
        }
    ...
    satisfied(ports) {
        ...
        for (const port of ports) {
            ...
            for (const source of port.sources) {
                if (!port.queue.some((message) => message.from === source))
                    return false;
            }
        }
