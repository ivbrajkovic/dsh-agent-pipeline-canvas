...
import { projectNodes } from "./projection.js";
...
class RunExecutor {
     ...
     */
    async pauseLoop(firing, agent, agentById, upstream, outputsById) {
        let current = firing;
        while (true) {
            ...
            if (cmd.action === "rerun") {
                ...
                try {
                    if (this.canContinuable) {
                        ...
                        rerunFiring = this.openFiring(nodeId, current, current.input);
                        this.record.state = "running";
                        this.currentFiringId = rerunFiring.firingId;
                        await this.commit();
                        const end = await this.runContinuableEpoch(rerunFiring, agent, agentById, rerunFiring.input);
                        if (end === null)
                            return false;
                        ...
                        if (outcome.error) {
                            ...
                            if (outcome.childSessionId !== undefined)
                                rerunFiring.childSessionId = outcome.childSessionId;
                            outputsById[nodeId] = rerunFiring.output;
                        }
                    }
                }
    ...
    async run(resumeFromPause) {
        try {
            ...
            for (let i = 0; i < order.length; i++) {
                ...
                if (projected.nodes[id]?.status === "done")
                    continue;
                ...
                if (pausedIndex === i) {
                    ...
                    if (!(await this.pauseLoop(pausedFiring, agent, agentById, classified.upstream[id] ?? [], outputsById)))
                        break;
                    continue;
                }
                ...
                try {
                    ...
                    if (useContinuable) {
                        const end = await this.runContinuableEpoch(firing, agent, agentById, firing.input);
                        if (end === null)
                            break;
                        ...
                        if (!(await this.pauseLoop(firing, agent, agentById, upstream, outputsById)))
                            break;
                        continue;
                    }
