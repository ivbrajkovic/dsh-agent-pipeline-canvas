...
class RunExecutor {
     ...
     */
    async pauseAt(initial, agent, agentById, inputs) {
        let openTurn = () => { };
        const turn = new Promise((resolve) => { openTurn = resolve; });
        const entry = { firing: initial, agent, agentById, inputs, turn, openTurn };
     ...
     * arrives in P6).
     */
    async emitOutput(nodeId, firing) {
    ...
    async runFiring(nodeId, messages, agentById) {
        ...
        try {
            ...
            if (useContinuable) {
                ...
                if (outcome.error && agent.breakpoint !== true)
                    return; // P3: recorded, not gating (fail-fast is P6)
            }
        ...
        try {
            ...
            if (resumeFromPause && projected.pausedFiring !== undefined) {
                ...
                if (agent !== undefined) {
                    ...
                    if (released !== null)
                        await this.emitOutput(pausedFiring.nodeId, released);
                }
            }
            ...
            while (!this.signal.aborted) {
                ...
                for (const nodeId of kernel.fireableNodes()) {
                    if (this.signal.aborted || kernel.halted || kernel.inFlight >= kernel.maxInFlight)
                        break;
