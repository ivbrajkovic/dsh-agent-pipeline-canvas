...
import { lowerControls } from "./controls.js";
...
class RunExecutor {
     ...
     * one came back — content bindings evaluate against it (a continuable
     * firing has none, so a bound breakpointed node emits quietly except on a
     * `$count` row, which tests `firing.seq` instead — the one way a released
     * breakpointed firing can still select a port). Only reached for
     * `completed` firings: a failed firing returns before this point (P6 —
     * nothing downstream of a failure runs), and a firing without an output
     * emits nothing — downstream starves and is reported at quiescence.
     */
    async emitOutput(nodeId, firing, structured) {
        if (typeof firing.output !== "string")
            return;
        const emission = this.kernel !== null
            ? this.kernel.emit(nodeId, firing.output, structured, firing.seq)
            : { ports: [], drops: [] };
