window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		function branchRows(branch) {
			if (branch == null || typeof branch !== "object") return [];
			const rec = branch;
			if (Array.isArray(rec.conditions)) return rec.conditions.filter((c) => c != null && typeof c === "object" && !Array.isArray(c));
			if (rec.field === void 0 && rec.value === void 0 && rec.op === void 0) return [];
			return [{
				...rec.field !== void 0 ? { field: rec.field } : {},
				...rec.value !== void 0 ? { value: rec.value } : {},
				...rec.op !== void 0 ? { op: rec.op } : {}
			}];
		}
		/**
		...
		function validateBranches(controlId, branches, errors) {
			...
			branches.forEach((entry, index) => {
				...
				rows.forEach((row, rowAt) => {
					...
					if (valued && (typeof row.field !== "string" || row.field.length === 0)) errors.push({
						code: "if-branch-invalid",
						message: `control "${controlId}" branch ${label}${rowLabel} compares a value but names no field`
					});
					if (row.op !== void 0 && !BRANCH_OPS$1.includes(row.op)) errors.push({
						code: "if-branch-invalid",
						message: `control "${controlId}" branch ${label}${rowLabel} has an unknown op "${argStr$1(row.op)}" (expected "==" or ">=")`
					});
					if (row.op === ">=" && !Number.isFinite(Number(row.value))) errors.push({
						code: "if-branch-invalid",
						message: `control "${controlId}" branch ${label}${rowLabel} compares with ">=" but its value is not a finite number`
					});
					if (!valued) {
						if (catchAllSeen || !(isLastBranch && rowAt === rows.length - 1)) errors.push({
							code: "if-branch-invalid",
							message: `control "${controlId}" branch ${label}${rowLabel} is a catch-all but not last — the catch-all must be the final condition of the final branch`
						});
						catchAllSeen = true;
					}
				});
		...
		function countsOnly(branches) {
			...
			for (const entry of branches) {
				...
				for (const row of branchRows(entry)) {
					...
					valued += 1;
					if (row.field !== "$count") return false;
				}
			}
			return valued > 0;
		}
		...
		function lowerControls(graph) {
			...
			for (const entry of raw) {
				...
				for (const branch of Array.isArray(control.branches) ? control.branches : []) {
					...
					if (rows.length === 0) bindings.push({ port: name });
			...
			return {
				...
				...controls.length > 0 ? { controls: controls.map((k) => ({
					...
					branches: k.branches.map((b) => {
						const side = b.side !== void 0 && b.side !== "right" ? { side: b.side } : {};
						const rows = branchRows(b).map((row) => ({
							...typeof row.field === "string" && row.field.length > 0 ? { field: row.field } : {},
							...row.value !== void 0 && row.value !== "" ? { value: row.value } : {},
							...row.op === ">=" ? { op: row.op } : {}
						}));
		...
		*/
			...
			for (const entry of raw) {
				...
				const branches = Array.isArray(rec.branches) ? rec.branches.map((b) => {
					...
					const rows = branchRows(b).map((row) => ({
						...
						...row.value === void 0 ? {} : { value: String(row.value) },
						...row.op === ">=" ? { op: ">=" } : {}
					}));
					const base = {
		...
		function AgentConfigPanel({ agent, onSave, onClose }) {
			...
			const schemaProps = (() => {
				...
				const props = JSON.parse(schemaTrimmed).properties;
				return props != null && typeof props === "object" && !Array.isArray(props) ? Object.keys(props).filter((k) => k.length > 0 && k !== "$count") : [];
			})();
			const bindingFields = ["$count"].concat(schemaProps);
									...
									/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
										...
										children: [
											...
											/* @__PURE__ */ (0, react_jsx_runtime.jsx)("datalist", {
												id: "pipeline-binding-fields",
												children: bindingFields.map((field) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("option", {
													value: field,
													children: field === "$count" ? "iteration count (built-in)" : field
												}, field))
											}),
											bindingRows.length === 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
												className: "config-hint",
												children: [
													"Without bindings a firing emits on every output port. Add rules to route the structured output — first match wins; an empty value is the catch-all, keep it last. The field may be a schema property or the reserved ",
													/* @__PURE__ */ (0, react_jsx_runtime.jsx)("code", { children: "$count" }),
													" (the firing's 1-based sequence — the only built-in)."
												]
		...
		function ControlConfigPanel({ control, warnings, rowWarnings, fieldOptions, onSave, onClose }) {
			const [name, setName] = react.useState(control.name ?? "");
			const [gates, setGates] = react.useState(control.branches.map((b) => ({
				name: b.name,
			...
			function setGate(index, patch) {
				setGates((prev) => prev.map((g, i) => i === index ? {
					...g,
					...patch
				} : g));
			}
			function setCondition(gateIndex, condIndex, patch) {
				setGates((prev) => prev.map((g, i) => i !== gateIndex ? g : {
					...g,
					conditions: g.conditions.map((c, j) => j === condIndex ? {
						...c,
						...patch
					} : c)
				}));
			}
			function addCondition(gateIndex) {
				setGates((prev) => prev.map((g, i) => i !== gateIndex ? g : {
					...g,
					conditions: g.conditions.concat([{
						field: "",
						op: "==",
						value: ""
					}])
			...
			function removeCondition(gateIndex, condIndex) {
				setGates((prev) => prev.map((g, i) => i !== gateIndex ? g : {
					...g,
					conditions: g.conditions.filter((_, j) => j !== condIndex)
				}));
			}
			...
			function kept() {
				return gates.map((g) => ({
					...g,
					conditions: g.conditions.filter((c) => c.field.trim().length > 0 || c.value.trim().length > 0)
			...
			keptGates.forEach((gate, gateIndex) => {
				...
				if (gate.conditions.length === 0) {
					if (gateIndex < keptGates.length - 1) shapeError = `Branch ${gateLabel} is a catch-all (no conditions) — it must stay the last branch.`;
					return;
				}
				gate.conditions.forEach((cond, condIndex) => {
					...
					if (value.length === 0 && !(gateIndex === keptGates.length - 1 && condIndex === gate.conditions.length - 1)) shapeError = `Branch ${gateLabel}${rowLabel} is a catch-all (empty value) — it must stay the last condition of the last branch.`;
				});
			});
			if (shapeError === null && keptGates.length === 0) shapeError = "Add at least one branch.";
			function assemble() {
				return keptGates.map((gate) => {
					const gateName = gate.name.trim();
					const side = gate.side !== "right" ? { side: gate.side } : {};
					if (gate.conditions.length === 0) return {
						name: gateName,
						...side
					};
			...
			const fieldList = ["$count"].concat(fieldOptions ?? []);
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: "pipeline-config-overlay",
				onPointerDown: (e) => {
					e.stopPropagation();
				...
				children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							className: "config-hint",
							children: [
								"Branches evaluate top to bottom on the feeding agent's structured output — the first MATCHING branch wins, and a branch fires when ANY of its conditions matches (so one gate can carry several: a verdict check and a count check together). The op picker carries the loop vocabulary: ",
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("code", { children: ">=" }),
								" compares numerically. The catch-all — a branch with no conditions, or a last condition with an empty value — must stay last. Wire each branch tick to the agent that handles it."
							]
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							className: "config-row",
							children: [
								...
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
									...
									children: [
										...
										fieldOptions !== void 0 && fieldOptions.length > 0 ? ": " + fieldOptions.join(", ") : " — none declared yet."
									]
								}),
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("datalist", {
									id: "pipeline-branch-fields",
									children: fieldList.map((field) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("option", {
										value: field,
										children: field === "$count" ? "iteration count (built-in)" : field
									}, field))
								}),
								gates.map((gate, gateIndex) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
									...
									children: [
										...
										gate.conditions.length === 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
											className: "config-hint config-branch-else",
											children: "catch-all — matches any structured result (fires only if no branch above matched)"
										}) : null,
										gate.conditions.map((cond, condIndex) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
											className: "config-mini-row config-condition",
											children: [
												/* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
													value: cond.field,
													placeholder: "field",
												...
												/* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
													value: cond.value,
													placeholder: cond.field.trim() === "$count" ? "iterations — e.g. 3" : "value — empty = catch-all",
													title: cond.field.trim() === "$count" ? "The iteration threshold — with >= the row matches from this firing number on" : "Value the field must equal (compared as text). Empty matches any structured result — the catch-all, kept last.",
													style: { flex: "1 1 32%" },
													onChange: (e) => {
										...
										}), cond.field.trim() === "$count" ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
											className: "config-hint",
											children: ["count " + cond.op + " " + (cond.value.trim().length > 0 ? cond.value.trim() : "…") + " → " + (gate.name.trim().length > 0 ? gate.name.trim() : "…"), " — iteration count: the feeding agent's firing sequence for this firing (1-based); with >= it escapes the loop from the threshold on"]
										}) : null] }, condIndex)),
										/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
										...
										rowWarnings !== void 0 && rowWarnings[gate.name.trim()] ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
											className: "config-warning",
											children: rowWarnings[gate.name.trim()]
										}) : null
									]
								}, gateIndex)),
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
		...
		function PipelineView({ sessionId, useSessions, useWorkspaces, inputActions, openView, services, onDismiss }) {
			...
			* commit — the same patches any other landing applies). */
			...
			function branchShadowWarnings(control) {
				...
				const out = {};
				const rows = control.branches.flatMap((branch) => branchRows(branch).map((row) => ({
					name: String(branch.name ?? ""),
					row
				})));
				rows.forEach((entry, index) => {
					if (entry.row.field !== "$count") return;
					...
					for (let above = 0; above < index; above++) {
						...
						if (connections.some((c) => c.source === control.id && c.sourcePort === prior.name && onCycle.has(c.target))) {
							out[entry.name] = prior.name === entry.name ? `the $count row "${entry.name}" sits below an earlier condition of the same branch, which wires back into the loop and shadows it` : `the $count row "${entry.name}" sits below row "${prior.name}", which wires back into the loop and shadows it`;
							break;
						}
					}
				});
			...
			function controlFieldOptions(control) {
				const feeder = connections.find((c) => c.target === control.id)?.source;
				const schema = agents.find((a) => a.id === feeder)?.settings?.outputSchema;
				const props = schema != null && typeof schema === "object" && !Array.isArray(schema) ? schema.properties : void 0;
				return props != null && typeof props === "object" && !Array.isArray(props) ? Object.keys(props).filter((k) => k.length > 0 && k !== "$count") : [];
			}
			const controlNodes = controls.map((control) => {
				...
				return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							className: "node-name",
							children: control.name ?? (isIf ? "if" : control.kind)
						}),
					...
					configControl ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(ControlConfigPanel, {
						...
						onSave: (name, branches) => {
							setControls((prev) => prev.map((k) => k.id === configControl.id ? {
								...k,
								branches,
								...name.length > 0 ? { name } : { name: void 0 }
							} : k));
