		...
		function PipelineView({ sessionId, useSessions, useWorkspaces, inputActions, openView, services, onDismiss }) {
			...
			function run(text, files) {
				...
				fetch(ENDPOINT + "/run", {
					...
					runAbortRef.current = null;
					setRunning(false);
					const stopped = err !== null && err.name === "AbortError";
					setRunResult({
						ok: false,
						error: stopped ? "Run stopped — the in-flight agent was interrupted." : String(err)
					});
					setResultOpen(true);
				});
			}
