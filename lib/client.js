	...
	factory: (require) => {
		...
		* words no longer rendered as a node tag. */
		function controlRunTitle(runState) {
			return runState.state === "fired" ? "The decision fired — branch " + runState.chosen.join(", ") : runState.state === "quiet" ? "The feeding agent's result matched no branch — nothing downstream of the if ran" : runState.state === "armed" ? "No branch decision recorded — the feeding agent's last firing never reached emission" : "The run has not reached this decision yet";
		}
		...
		* dot's idiom. */
		...
		function PipelineView({ sessionId, useSessions, useWorkspaces, inputActions, openView, services, onDismiss }) {
			...
			function commitConnection(conn) {
				...
				if (verdict.inputPorts !== void 0) setAgents((prev) => prev.map((a) => a.id === conn.target ? {
					...a,
					inputPorts: verdict.inputPorts
				} : a));
			...
			function deleteNode(nodeId) {
				...
				if (controls.some((k) => k.id === nodeId)) {
					...
					if (selectedConn !== void 0 && (selectedConn.source === nodeId || selectedConn.target === nodeId)) setSelectedEdgeId(null);
				} else {
					...
					if (selectedConn !== void 0 && (selectedConn.source === nodeId || selectedConn.target === nodeId || dying.has(selectedConn.source) || dying.has(selectedConn.target))) setSelectedEdgeId(null);
				}
			...
			function nearestInputPort(nodeId, pt) {
