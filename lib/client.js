window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
			...
			async function continueInNewSession() {
				...
				try {
					...
					if (uiWorkspace && typeof uiWorkspace.connectWorkspace === "function") {
						const ws = workspaceItems.find((w) => Array.isArray(w.sessionIds) && w.sessionIds.indexOf(sessionId) !== -1) || (cwd ? workspaceItems.find((w) => w.path === cwd) : void 0);
						if (ws && typeof ws.workspaceId === "string") newId = await uiWorkspace.connectWorkspace(ws.workspaceId);
					}
					if (newId === void 0 && sessions && typeof sessions.create === "function" && cwd) newId = await sessions.create({ cwd });
		...
		function PipelineTriggerRow({ wide }) {
			const open = react.useSyncExternalStore(panelGate.subscribe, panelGate.get);
			return react.createElement("div", { className: "pipeline-trigger" + (wide ? "" : " rail") }, react.createElement("button", {
				type: "button",
			...
			}, react.createElement(PipelineGlyph, { size: wide ? 16 : 18 }), wide ? react.createElement("span", { className: "pipeline-trigger-label" }, "Pipelines") : null));
		}
		/**
		...
		*/
		function PipelinePanelEntry({ useSessions, services }) {
		...
		function PipelinePanel({ useSessions, services }) {
		...
		function apply(ctx) {
			...
			ctx.slots.inject("sidebar.footer.action", () => ctx.slots.register({
				...
				id: "pipeline-trigger",
				order: 20
			}, (props) => PipelineTriggerRow({ wide: props.wide })));
