...
export function continuableSupported(ctx) {
 ...
 */
function applySettings(request, agent, options) {
    const settings = agent?.settings;
    if (settings?.agentOptions != null)
        request.agentOptions = settings.agentOptions;
    if (options.includeOutputSchema && settings?.outputSchema != null)
...
export function agentLabel(agent, id) {
 ...
 */
export async function runOneAgent(ctx, options) {
 ...
 */
export async function startContinuableAgent(ctx, options) {
    ...
    applySettings(request, agent, { includeOutputSchema: false });
    const start = ctx.subagents.startContinuable;
    if (typeof start !== "function")
        throw new Error("continuable subagents are not available in this deployment");
 ...
 */
export async function steerContinuableAgent(ctx, options) {
    const followup = ctx.subagents.followup;
    if (typeof followup !== "function")
        throw new Error("steering requires continuable subagent support");
    const signal = options.signal ?? new AbortController().signal;
